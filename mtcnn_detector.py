...
class MtcnnDetector(object):
    ...
    def __init__(self,
        ...
        for i in range(num_worker):
            ...
            #workner_net = mx.mod.Module.load(models[0], 1,label_names=[],context=ctx)
            self.PNets.append(workner_net)
        self.RNet = mx.model.FeedForward.load(models[1], 1, ctx=ctx)
        #self.RNet =mx.mod.Module.load(models[1], 1,label_names=[],context=ctx)
        self.ONet = mx.model.FeedForward.load(models[2], 1, ctx=ctx)
        #self.ONet = mx.mod.Module.load(models[2], 1,label_names=[],context=ctx)
        self.LNet = mx.model.FeedForward.load(models[3], 1, ctx=ctx)
        #self.LNet = mx.mod.Module.load(models[3], 1,label_names=[],context=ctx)
    ...
    def extract_image_chips(self, img, points, desired_size=256, padding=0):
        ...
        for p in points:
            ...
            #mean_face_shape_x =[0.34191608, 0.6565339 , 0.500225  , 0.3709759 , 0.631517  ]
            #mean_face_shape_y = [0.2119465, 0.2119465, 0.628106, 0.780233, 0.780233]
            #mean_face_shape_y =[0.4615741 , 0.45983392, 0.6405054 , 0.82469195, 0.8232509 ]
            from_points = []
            to_points = []
